**ASTRA — nihai tamir edilmiş komut ve çalıştırılabilir paket, v1.3**

6 Eylül 2026. Bu sürüm, astra_tamir_v1_2.md içindeki kaynak/inceleyici ve karşılaştırma bağlantısını tamamlar. Tam çalışma komutu, uygulama kodu ve gerçek test kayıtları aşağıdadır.

**Tamir:** Controller.finalize artık TrustedHost olmadan başarı vermez. Sözleşmedeki karşılaştırmaları gerçek snapshotlarla çağırır; somut aday, kaynak metinleri, hesap kanıtları ve kıyas sonuçlarını inceleyici sürecine gönderir. Eksik/olumsuz/yanlış alıntılı/eski veya yeniden kullanılan inceleme kapıyı kapatır. Dışarıdan yazılmış true bayrakları gerçek çağrının yerine geçmez.

**Doğrulama:** Python 3.12.13 ile 113 test geçti; başarısızlık, hata ve atlama yok. Dört gerçek CLI senaryosunda geçerli örnek kabul edildi; farklı yöntem, olumsuz anlam incelemesi ve eksik inceleyici FAIL_CLOSED verdi. Olay kayıtları verification dizinindedir.

İşçiler ve test inceleyicileri TEST_FIXTURE'dır. Yanlış anlamlı girdiler için verilen sabit ret kararının gerçek çalıştırıcıda uygulanması sınanmıştır; bu, bir LLM'nin hatayı bağımsız bulma başarısı değildir. Gerçek OpenAI Responses adaptörü eklendi ve HTTP yanıt örnekleriyle sınandı. Ortamda API anahtarı olmadığı için canlı Astra Max çağrısı yapılmadı. Kaynak erişimi yerel UTF-8 snapshot dosyalarıyla sınırlıdır; web sitesinin köken doğrulaması ve üretim izolasyonu onaylanmış değildir.

Durum: REPAIR_STATUS=IMPLEMENTED_AND_LOCAL_TESTED; MODE=LOCAL_TEST; FINAL_STATUS=LOCAL_CHECKS_PASSED; MODEL_EVAL=NOT_RUN; PRODUCTION_APPROVAL=false. Yerel tamir tamamlanmıştır; genel doğruluk garantisi verilmez.

**Çalışma komutu**

BEGIN ASTRA_COMMAND ile END ASTRA_COMMAND arasını kullanıcı göreviyle birlikte uygula. Belgeyi yüklemek Python kodunu çalıştırmaz ve platformun model ayarını değiştirmez. Program kullanımı için çıkarma yönergesini ve gömülü README.md'yi izle.
...
Python 3.11+ ve POSIX gerekir. Aşağıdaki komut bu belgenin bulunduğu dizinde çalıştırılır. Yeni dizin oluşturur; var olan dosyaların üzerine yazmaz. Envanteri, boyutları ve hash'leri kontrol eder. Hash kimlik doğrulama imzası değildir.
...
doc = Path("astra_tamir_v1_3.md").read_text(encoding="utf-8")
blocks = re.findall(r"<!-- BEGIN FILE: ([^>]+) -->\n```(?:python|json|markdown|text)\n(.*?)\n```\n<!-- END FILE -->", doc, re.S)
expected = {'history/v1_2/test_run.txt', 'CHANGELOG.md', 'verification/test_result.json', 'README.md', 'verification/cli_runs/semantic_rejection/result.json', 'verification/cli_runs/reviewer_missing/source_0.txt', 'verification/cli_runs/method_mismatch/source_1.txt', 'verification/cli_runs/method_mismatch/result.json', 'tests/test_astra.py', 'verification/cli_runs/reviewer_missing/result.json', 'verification/source_review_notes.json', 'history/v1_2/test_result.json', 'astra_command.md', 'verification/cli_runs/semantic_rejection/source_0.txt', 'astra_compare.py', 'tests/worker_fixture.py', 'repair_contract.json', 'history/v1_2/baseline_test_run.txt', 'history/v1_2/audit_records.json', 'BUNDLE_MANIFEST.json', 'verification/cli_runs/semantic_rejection/config.json', 'verification/cli_runs/method_mismatch/config.json', 'verification/cli_runs/valid/source_0.txt', 'tests/semantic_reviewer_fixture.py', 'demo_local.py', 'tests/test_host_integration.py', 'tests/test_plugin_router.py', 'verification/cli_runs/reviewer_missing/config.json', 'verification/cli_runs/semantic_rejection/source_1.txt', 'verification/cli_runs/valid/config.json', 'prompt_spec.md', 'verification/cli_runs/valid/source_1.txt', 'astra_plugin_router.py', 'verification/cli_runs/valid/result.json', 'verification/cli_summary.json', 'tests/test_goal_regressions.py', 'astra_plugin_atlas.json', 'tests/test_openai_reviewer.py', 'tests/goal_worker_fixture.py', 'astra_openai_reviewer.py', 'astra_host.py', 'astra_run.py', 'verification/cli_runs/method_mismatch/source_0.txt', 'verification/cli_runs/reviewer_missing/source_1.txt', 'tests/research_worker_fixture.py', 'verification/test_run.txt', 'tests/host_fixture_support.py', 'tests/test_comparison.py', 'astra_reference.py'}
if len(blocks) != len(expected) or {name for name, _ in blocks} != expected:
    raise SystemExit("Embedded source inventory mismatch")
for name, _ in blocks:
    if Path(name).is_absolute() or ".." in Path(name).parts:
        raise SystemExit("Unsafe embedded path")
# v1.3 preserves exact bytes, including files without a trailing newline.
by_name = dict(blocks)
manifest = json.loads(by_name["BUNDLE_MANIFEST.json"])
if {r["path"] for r in manifest["files"]} != expected - {"BUNDLE_MANIFEST.json"}:
...
for record in manifest["files"]:
    ...
    if len(raw) != record["size_bytes"] or hashlib.sha256(raw).hexdigest() != record["sha256"]:
        raise SystemExit("Content hash mismatch: " + record["path"])
target = Path("astra_final_bundle_v1_3")
target.mkdir(exist_ok=False)
for name, contents in by_name.items():
    path = target / name
...
```bash
cd astra_final_bundle_v1_3
python3 -m unittest discover -s tests -p 'test_*.py' -v
python3 demo_local.py --case valid --output-dir demo_valid
```

Gerçek çalıştırıcı: `python3 astra_run.py config.json --output result.json`. Gerçek inceleyici için host ortamında OPENAI_API_KEY ve README'deki EXTERNAL_MODEL yapılandırması gerekir. Anahtar yokken örnek inceleyiciye sessiz geçiş yoktur; kapı kapanır. ZIP aynı dosyaları hazır olarak içerir.

OpenAI arayüzü için [Structured Outputs](https://developers.openai.com/api/docs/guides/structured-outputs), [reasoning rehberi](https://developers.openai.com/api/docs/guides/reasoning) ve [Astra model belgesi](https://developers.openai.com/api/docs/models/gpt-6-astra) kontrol edildi. Bu belgeler arayüzü destekler; canlı çağrı veya kusursuz anlam incelemesi yapıldığını kanıtlamaz.
...
{
  "version": "1.3",
  "created_at": "2026-09-06T12:48:44.990496+00:00",
  "files": [
    {
      "path": "CHANGELOG.md",
      "sha256": "38d9c424a62c28075d949ef3572c7d069b0de8ec99689ee9658caba04502c2f7",
      "size_bytes": 1512
    },
    {
      "path": "README.md",
      "sha256": "93f53e904cf1986d0b0bd1956d1f40e4029aa27555140d3f006e175a07e70dd4",
      "size_bytes": 6429
    },
    {
      "path": "astra_command.md",
      "sha256": "ea40968bbdbdcbf6778f34af5abef24592b521bcd7811da33d1c1c062e8cd8fd",
      "size_bytes": 55213
    },
    {
      "path": "astra_compare.py",
      "sha256": "f41655814caa5b1fab2d229ad15d5b1cb39bfbbf454d3d38f7c4dbf055dd5c64",
      "size_bytes": 4375
    },
    {
      "path": "astra_host.py",
      "sha256": "437f2de445d016ff367ee2ec7aef96631dab687eefe38bc4a43de403771847ab",
      "size_bytes": 18479
    },
    {
      "path": "astra_openai_reviewer.py",
      "sha256": "377a0c146542deca76d0b669f91b538dd99b4e0bf6af2136413da6e8058e859d",
      "size_bytes": 4693
    },
    {
    ...
    },
    {
      ...
      "sha256": "b58d7bec97759f3debb8e40d583334e2181e615a6b46694845afecb548f60402",
      "size_bytes": 29161
    },
    {
      "path": "astra_run.py",
      "sha256": "761dda3827c0e515993be76a34e292020b8a0965cea2293dba7df100a3ebda41",
      "size_bytes": 2974
    },
    {
      "path": "demo_local.py",
      "sha256": "0a0e1da1c7f77008a6326ea936a88ee83af4ef6481ef232a1c7d19bf9cd5b285",
      "size_bytes": 3443
    },
    {
    ...
    {
      "path": "prompt_spec.md",
      "sha256": "2a02f8293c151d00a066ed6ab01627ffe2d9c172cfff06bc9d21eb4629636925",
      "size_bytes": 1316
    },
    {
      "path": "repair_contract.json",
      "sha256": "bbe054cd37460e0879d828a24fc15aef5d656adda791049cf3e83c644f38e2ae",
      "size_bytes": 1393
    },
    {
      "path": "tests/goal_worker_fixture.py",
      "sha256": "562ef87d59f24c5d1b48cd47d798eacacffe041852004e3558fa0a04cd3a0f47",
      "size_bytes": 952
    },
    {
      "path": "tests/host_fixture_support.py",
      "sha256": "8d42d9a5eea54fd842ac8fdeb87ad4472bad00e6819e2fbfdf5424c48f067424",
      "size_bytes": 1784
    },
    {
      "path": "tests/research_worker_fixture.py",
      "sha256": "8da0c045818e01f04bb873259306cfc6cbd1175765e38050a49aeab8029d1a10",
      "size_bytes": 750
    },
    {
      "path": "tests/semantic_reviewer_fixture.py",
      "sha256": "c2d00bc90e1ad637adb1aede98cfe7e7cdd255ff7a18ab7db72829aaa81d0296",
      "size_bytes": 3108
    },
    {
      "path": "tests/test_astra.py",
      "sha256": "0a4170872af92661a1eccb1eef70a3cbe33f0b01ab22134d81fb43de3b898b09",
      "size_bytes": 12213
    },
    {
      "path": "tests/test_comparison.py",
      "sha256": "b63740fbde12f19adc7190ecdaa5c3d06017a1003c933cf79df0b538058c80eb",
      "size_bytes": 5801
    },
    {
    ...
    },
    {
      ...
      "sha256": "ca330509dd8af9988790d7be4e3caa2b558b2aa3ad9b5b61c6d21b4e2ae279a4",
      "size_bytes": 9902
    },
    {
      "path": "tests/test_openai_reviewer.py",
      "sha256": "00a6ceaf2d0b59e486a59bad433bf87bc4b14f09ed09a821dcc92f31d473823d",
      "size_bytes": 4440
    },
    {
    ...
    },
    {
      ...
      "sha256": "af398412a80d0110a079a27772935dbe3c78eb50b5549d52726c4a6b00e2f379",
      "size_bytes": 2780
    },
    {
      "path": "verification/cli_runs/method_mismatch/config.json",
      "sha256": "3e931857f0f385f7bb5d08e401e676d99482a97d1eebdefe4e41ec0b6cd0b36c",
      "size_bytes": 3439
    },
    {
      "path": "verification/cli_runs/method_mismatch/result.json",
      "sha256": "da893eb346e9bbaf641a6e7a90da82d4fea215237b67be4f26e04f6bc7982319",
      "size_bytes": 2247
    },
    {
    ...
    },
    {
      ...
      "sha256": "2ffb35680d3de6badbe441886ac6912774834fba94c75a32be071d8f84100f5d",
      "size_bytes": 3104
    },
    {
      "path": "verification/cli_runs/reviewer_missing/result.json",
      "sha256": "5f8c1bc73074f1ce560bb9c10c46c5d83b441a46677be5ff5030ea99ce0d9d81",
      "size_bytes": 2633
    },
    {
    ...
    },
    {
      ...
      "sha256": "442528a95f1092cb01b672c66bf41e9230a9bdd6a54c75c76552941cc9ef942d",
      "size_bytes": 3477
    },
    {
      "path": "verification/cli_runs/semantic_rejection/result.json",
      "sha256": "7dabdc0a7b8f7b190821045bf5d204566e4c44b54e05a93646986f1ea5d0f539",
      "size_bytes": 2999
    },
    {
    ...
    },
    {
      ...
      "sha256": "56b8c57197aca49d7a0958a44ee7b2ad2af10cd4fe8a7153bde9092e36f5ca1c",
      "size_bytes": 3435
    },
    {
      "path": "verification/cli_runs/valid/result.json",
      "sha256": "09c177fd37400ee11d84b9026961961cf1ad2e23b0bbe3328f8d6895aed06eaf",
      "size_bytes": 13430
    },
    {
    ...
    },
    {
      ...
      "sha256": "f4b447ee7acc7cdd25c85eae9f5ba568078358b50b3ac745e3e9d5636e5677ec",
      "size_bytes": 1426
    },
    {
      "path": "verification/source_review_notes.json",
      "sha256": "af52b3e2828cb911dc8adc55e6db70c1d52f3ec764d31b8169c57c2df0aced93",
      "size_bytes": 1212
    },
    {
      "path": "verification/test_result.json",
      "sha256": "c38ee752b5fca05bd7f02b6f9d07e9fbf6df01935b7e2edb568e63fffc717c99",
      "size_bytes": 432
    },
    {
      "path": "verification/test_run.txt",
      "sha256": "a15f48e932b2d3ea243cc8bf9c782f8611360d39d628805c0556306e88d6b493",
      "size_bytes": 15107
    }
  ]
}
...
{
  "version": "1.3",
  "original_request": "Nihai tamir eddigin halini ver",
  "requirements": [
    {
      "id": "R1",
      "delivery": "Comparison gate mandatory in Controller.finalize, bound to frozen task/claims/sources/results",
      "status": "VERIFIED",
      "evidence": [
        "verification/test_run.txt",
        "verification/cli_summary.json"
      ]
    },
    {
      "id": "R2",
      "delivery": "Actual semantic reviewer invocation mandatory; missing/forged/negative/stale evidence blocks",
      "status": "VERIFIED",
      "evidence": [
        "verification/test_run.txt",
        "verification/cli_runs/valid/result.json",
        "verification/cli_runs/semantic_rejection/result.json",
        "verification/cli_runs/reviewer_missing/result.json"
      ]
    },
    {
      "id": "R3",
      "delivery": "Self-contained updated command and executable package with real test evidence",
      "status": "VERIFIED",
      "evidence": [
        "astra_command.md",
        "README.md",
  ...
  "production_approval": false,
  "model_eval": "NOT_RUN",
  "updated_at": "2026-09-06T12:44:44.824302+00:00",
  "task_status": "COMPLETE",
  "repair_status": "IMPLEMENTED_AND_LOCAL_TESTED",
  "final_status": "LOCAL_CHECKS_PASSED",
  "live_model_dependency": "OPENAI_API_KEY not available in this environment",
  "tests_run": 113
}
...
```json
{
  ...
  "sources": [
    {
      "source_id": "s0",
      "path": "/workspace/scratch/52fc0e57f193/astra_final_bundle_v1_3/verification/cli_runs/method_mismatch/source_0.txt",
      "kind": "TOOL",
      "as_of": "2026-09-06T12:36:16.384194+00:00",
      "valid_until": "2026-09-06T13:36:17.384194+00:00"
    },
    {
      "source_id": "s1",
      "path": "/workspace/scratch/52fc0e57f193/astra_final_bundle_v1_3/verification/cli_runs/method_mismatch/source_1.txt",
      "kind": "TOOL",
      "as_of": "2026-09-06T12:36:16.384194+00:00",
      "valid_until": "2026-09-06T13:36:17.384194+00:00"
    }
  ...
  "workers": {
    "a": {
      ...
      "argv": [
        ...
        "/workspace/scratch/52fc0e57f193/astra_final_bundle_v1_3/tests/research_worker_fixture.py"
      ]
    },
    "b": {
      "role": "counterexample",
  ...
  "reviewer": {
    "argv": [
      "/opt/codex/runtimes/codex-primary-runtime/dependencies/python/bin/python3",
      "/workspace/scratch/52fc0e57f193/astra_final_bundle_v1_3/tests/semantic_reviewer_fixture.py",
      "pass"
    ],
    "kind": "TEST_FIXTURE",
    "model": "fixture-model",
...
```json
{
  ...
  "events": [
    {
      "detail": {
        "phase_id": "d8ae8a9cea0944578c1f7795ebfa39a9",
        "reply_digest": "sha256:bd04788810513df67662e04b72525b02626477302a2125265ed5650bdf0881d9",
        "worker_id": "a"
      },
      "kind": "READY",
      "previous": null,
      "run_id": "adab0575bf964e539613b07dd199c4bc"
    },
    {
      "detail": {
        "phase_id": "d8ae8a9cea0944578c1f7795ebfa39a9",
        "reply_digest": "sha256:da3d38fa0c553bf4b187da0d22c071ace7228f820d96de4117f2750e1e85fc42",
        "worker_id": "b"
      },
      "kind": "READY",
      "previous": "sha256:b64be9ce5788ed945007ce99cdb2d2929acb94d9f77eea187ce24a73194afbc4",
      "run_id": "adab0575bf964e539613b07dd199c4bc"
    },
    {
      "detail": {
        "phase_id": "d8ae8a9cea0944578c1f7795ebfa39a9",
        "reply_digest": "sha256:54a961e470464f1b27b04024c608de5d924557e450b22016b0c8b20f40e10a7f",
        "worker_id": "c"
      },
      "kind": "READY",
      "previous": "sha256:493e09b01432c1572cf4b0cbc03fb04bf2613cf610db196e9ca0b32553e06a48",
      "run_id": "adab0575bf964e539613b07dd199c4bc"
    },
    {
      "detail": {
      ...
      "kind": "PHASE_CHECK",
      "previous": "sha256:fe9ffae8a0079fd4b81446c0303db4435cecb2c4292be45f43c544edfa4f2d0f",
      "run_id": "adab0575bf964e539613b07dd199c4bc"
    },
    {
      "detail": {
      ...
      "previous": "sha256:10e89629be4f0a674ea694d7113d63691594e668d7fdb75275f62f74b4813468",
      "run_id": "adab0575bf964e539613b07dd199c4bc"
    },
    {
      "detail": {
      ...
      "previous": "sha256:6c0fc0e43992bf06f2a40d18863b3e37400dd08a38ef6f9a706ef4d25c8eac61",
      "run_id": "adab0575bf964e539613b07dd199c4bc"
    }
  ],
  "production_approval": false
...
```json
{
  ...
  "sources": [
    {
      "source_id": "s0",
      "path": "/workspace/scratch/52fc0e57f193/astra_final_bundle_v1_3/verification/cli_runs/reviewer_missing/source_0.txt",
      "kind": "TOOL",
      "as_of": "2026-09-06T12:36:17.038417+00:00",
      "valid_until": "2026-09-06T13:36:18.038417+00:00"
    },
    {
      "source_id": "s1",
      "path": "/workspace/scratch/52fc0e57f193/astra_final_bundle_v1_3/verification/cli_runs/reviewer_missing/source_1.txt",
      "kind": "TOOL",
      "as_of": "2026-09-06T12:36:17.038417+00:00",
      "valid_until": "2026-09-06T13:36:18.038417+00:00"
    }
...
```json
{
  ...
  "events": [
    {
      "detail": {
        "phase_id": "f29f0d4272b14af4812d2bdcca801827",
        "reply_digest": "sha256:c9c03a10ace2568e226f85f045f74c4218a4144e9d52169bd8e797b3c28f7e26",
        "worker_id": "a"
      },
      "kind": "READY",
      "previous": null,
      "run_id": "4cc0691371e64e929340d61f20bdcf68"
    },
    {
      "detail": {
        "phase_id": "f29f0d4272b14af4812d2bdcca801827",
        "reply_digest": "sha256:60f53b099f6574f7adc5288d0c28a6f820eae25725ad36810f3e9ff35807cf3e",
        "worker_id": "b"
      },
      "kind": "READY",
      "previous": "sha256:84a45c168b57ea94feba032c52c9c10db9b61b9acce8aebc82955923f10cbd11",
      "run_id": "4cc0691371e64e929340d61f20bdcf68"
    },
    {
      "detail": {
        "phase_id": "f29f0d4272b14af4812d2bdcca801827",
        "reply_digest": "sha256:0824ac1ca574e53e176d86e1184000ff937e0c52e05fe96a59d582a8976d91e1",
        "worker_id": "c"
      },
      "kind": "READY",
      "previous": "sha256:15314c9be32f92e51c215f6e9e7da8bef2cdb4f99398499182bdb410a6b1e985",
      "run_id": "4cc0691371e64e929340d61f20bdcf68"
    },
    {
      "detail": {
      ...
      "kind": "PHASE_CHECK",
      "previous": "sha256:da4e8fcbd169e72eefafb126109a4d0916635545695ed77c3594376e708a526e",
      "run_id": "4cc0691371e64e929340d61f20bdcf68"
    },
    {
      "detail": {
      ...
      "kind": "COMPARISON_STARTED",
      "previous": "sha256:65c7bdbffb39b488a88cfa48390f8f7f36c63ae71ff2dff66b880f1d06da42e2",
      "run_id": "4cc0691371e64e929340d61f20bdcf68"
    },
    {
      "detail": {
        "comparison_digest": "sha256:ebd2978c097d1deedfeb78e61e18b1d966c5de807a2d1741d6d16dfe2140bf21",
        "requirement_id": "latency"
      },
      "kind": "COMPARISON_VALIDATED",
      "previous": "sha256:29b75d4eeacb56be11e1831ea2c24a1caf6581711607cf9475a70a70f093092b",
      "run_id": "4cc0691371e64e929340d61f20bdcf68"
    },
    {
      "detail": {
      ...
      "kind": "FINALIZATION_REJECTED",
      "previous": "sha256:188695fe48e089b050a0779b2f340c5242c88c0878c389a288ed1c84060e8ed6",
      "run_id": "4cc0691371e64e929340d61f20bdcf68"
    }
  ],
  "production_approval": false
...
```json
{
  ...
  "sources": [
    {
      "source_id": "s0",
      "path": "/workspace/scratch/52fc0e57f193/astra_final_bundle_v1_3/verification/cli_runs/semantic_rejection/source_0.txt",
      "kind": "TOOL",
      "as_of": "2026-09-06T12:36:16.708157+00:00",
      "valid_until": "2026-09-06T13:36:17.708157+00:00"
    },
    {
      "source_id": "s1",
      "path": "/workspace/scratch/52fc0e57f193/astra_final_bundle_v1_3/verification/cli_runs/semantic_rejection/source_1.txt",
      "kind": "TOOL",
      "as_of": "2026-09-06T12:36:16.708157+00:00",
      "valid_until": "2026-09-06T13:36:17.708157+00:00"
    }
...
```json
{
  ...
  "events": [
    {
      "detail": {
        "phase_id": "f6c8593bda1547c08b84ac58a38ea6d3",
        "reply_digest": "sha256:d855fe751d93bcf81c4b16ce1a2ff16cf5ddced862184f8aff448909355a0192",
        "worker_id": "a"
      },
      "kind": "READY",
      "previous": null,
      "run_id": "a11f5478f8e14358b390e44f41d1cfe6"
    },
    {
      "detail": {
        "phase_id": "f6c8593bda1547c08b84ac58a38ea6d3",
        "reply_digest": "sha256:b14f6d4020e1f0584cacf31e9f2f2ddb1817f29a2e82b73c3570181b6f78f56a",
        "worker_id": "b"
      },
      "kind": "READY",
      "previous": "sha256:8e4a744e05d7ea2a87921f2ee5275552440f882ca8ae997e4519e9d0f6ab413c",
      "run_id": "a11f5478f8e14358b390e44f41d1cfe6"
    },
    {
      "detail": {
        "phase_id": "f6c8593bda1547c08b84ac58a38ea6d3",
        "reply_digest": "sha256:a2866dd19d7a4101eac54febbdaf007604a5260081858cc2dbaefe9cbd2c3998",
        "worker_id": "c"
      },
      "kind": "READY",
      "previous": "sha256:ba4277bca3b4a114e45f077ddbb09898d76e70d88951fd7c1aa949039c66bb96",
      "run_id": "a11f5478f8e14358b390e44f41d1cfe6"
    },
    {
      "detail": {
      ...
      "kind": "PHASE_CHECK",
      "previous": "sha256:d11b367fc96fc5275c1ef6c0725cf9bf58e3829628599db323bddf0e3ca9e056",
      "run_id": "a11f5478f8e14358b390e44f41d1cfe6"
    },
    {
      "detail": {
      ...
      "kind": "COMPARISON_STARTED",
      "previous": "sha256:f04401ac8eb589c52e985e165d3c1d6f434dd26cc14a278ffae7adedafb6c995",
      "run_id": "a11f5478f8e14358b390e44f41d1cfe6"
    },
    {
      "detail": {
        "comparison_digest": "sha256:0b58551942adeef7ec589d2583d4a1d2e0af0d64e6a7aa15ddaef101fa2cb819",
        "requirement_id": "latency"
      },
      "kind": "COMPARISON_VALIDATED",
      "previous": "sha256:10bb91a6c608310aa3ea49cacfdd57968ea250177c4e4f514ebfdf4ca52ff8dc",
      "run_id": "a11f5478f8e14358b390e44f41d1cfe6"
    },
    {
      "detail": {
        "request_digest": "sha256:5a53cbb5e73b7d294365f38a1355865c1d14be158eec8a271add290f06d67479",
        "reviewer_kind": "TEST_FIXTURE"
      },
      "kind": "SEMANTIC_REVIEW_STARTED",
      "previous": "sha256:090617988b52d9eeb5722fd0aeea6325d6daaab362fd3004f36c0faaa4a5ad58",
      "run_id": "a11f5478f8e14358b390e44f41d1cfe6"
    },
    {
      "detail": {
      ...
      "kind": "FINALIZATION_REJECTED",
      "previous": "sha256:fe18f3d027c48841a7e4e9f63a6f8a47c17660e14fa5250996b99ec58246e9ec",
      "run_id": "a11f5478f8e14358b390e44f41d1cfe6"
    }
  ],
  "production_approval": false
...
```json
{
  ...
  "sources": [
    {
      "source_id": "s0",
      "path": "/workspace/scratch/52fc0e57f193/astra_final_bundle_v1_3/verification/cli_runs/valid/source_0.txt",
      "kind": "TOOL",
      "as_of": "2026-09-06T12:36:16.112160+00:00",
      "valid_until": "2026-09-06T13:36:17.112160+00:00"
    },
    {
      "source_id": "s1",
      "path": "/workspace/scratch/52fc0e57f193/astra_final_bundle_v1_3/verification/cli_runs/valid/source_1.txt",
      "kind": "TOOL",
      "as_of": "2026-09-06T12:36:16.112160+00:00",
      "valid_until": "2026-09-06T13:36:17.112160+00:00"
    }
...
```json
{
  ...
  "result": {
    ...
    "production_approval": false,
    "verification_boundary": "HOST_CAPTURED_LOCAL_SNAPSHOTS_AND_EXECUTED_GATES; reviewer judgment is not a truth or isolation guarantee",
    "run_id": "c15397adfca4466d91f2663a258c4135",
    "phase_digest": "sha256:efcdd96a22babd56f2f35f389c14669d3508ec56497e35c2c7a432aa48e0cede",
    "decision": {
      "action": "Report the observed latency comparison only.",
      "claim_ids": [
        "a:c1",
    ...
    "host_verification": {
      ...
      "comparison_results_digest": "sha256:ff870e335ef12787f7a4a6c3975cd3e446ca51836e5ae9bf7830c14cd202df6d",
      "comparisons": [
        {
          "claim_ids": [
            "a:c1",
          ...
          "requirement_id": "latency",
          "result": {
            "comparison_digest": "sha256:e88e9e02249695b1d9836ec7e870a667ef1abeeaad931405291bbb619fb67ca7",
            "context": {
              "definition": "median completion latency",
              "method": "same median harness",
              "metric": "latency",
            ...
            "rows": [
              {
                "access_record_id": "sha256:ce9a7d9f85d72e63e0e78d1bd7b882df3376a02f833f3586796284db9097479a",
              ...
              {
                "access_record_id": "sha256:557f51df323293f66dd94ea2168227f3170f8fe94c7f2608d900dd486c6f8be4",
      ...
      "contract_digest": "sha256:cd57e53c33d69e0359cfa79a2ca824f8e02f5a18d3b213e1dfdcc2d5ced37e8a",
      "created_at": "2026-09-06T12:36:17.307706+00:00",
      "host_receipt_id": "sha256:9f6615c0e85fe82bbe21186636293c05419774bb5e0f38323fa5b487a582ef36",
      "phase_digest": "sha256:efcdd96a22babd56f2f35f389c14669d3508ec56497e35c2c7a432aa48e0cede",
      "production_approval": false,
      "protocol": "ASTRA-HOST-1.3",
      "rendered_claims_digest": "sha256:b5873c1fc70ec4e4a1a9abdcfb10b4e0d179518202d874ac76f73ac00c624c3b",
      "request_digest": "sha256:be5e3651ebd3b1bee1375311bc62dbf608d19953b1b88351965c32f620f5abd3",
      "reviewer_kind": "TEST_FIXTURE",
      "reviewer_response": {
        "assessment": {
      ...
      "source_access_receipts": [
        {
          "access_record_id": "sha256:ce9a7d9f85d72e63e0e78d1bd7b882df3376a02f833f3586796284db9097479a",
          "captured_at": "2026-09-06T12:36:17.194747+00:00",
          "content_digest": "sha256:5262c6e580c881395b81be939bd532aaec3757d621c6f6e79a1326b33560d953",
          "locator": "file:///workspace/scratch/52fc0e57f193/astra_final_bundle_v1_3/verification/cli_runs/valid/source_0.txt",
        ...
        {
          "access_record_id": "sha256:557f51df323293f66dd94ea2168227f3170f8fe94c7f2608d900dd486c6f8be4",
          "captured_at": "2026-09-06T12:36:17.194883+00:00",
          "content_digest": "sha256:23ed7e88dd6e6d17813b096286bab897c4f94513a51c00bf4021359ae468b20a",
          "locator": "file:///workspace/scratch/52fc0e57f193/astra_final_bundle_v1_3/verification/cli_runs/valid/source_1.txt",
          "operation": "READ_LOCAL_UTF8_FILE",
          "source_id": "s1",
          "upstream_authentication_verified": false
        }
      ...
      "source_access_scope": "LOCAL_FILE_SNAPSHOT_READ",
      "source_registry_digest": "sha256:580d71a6c924df87995bf46e929ccaf3783ecf4d29f4b1c8f242b46bf76a44c3",
  ...
  "events": [
    {
      "detail": {
        "phase_id": "ad2278ae871a4f6f8e32c86fe835ca88",
        "reply_digest": "sha256:db66b74c3d893e0418da11109b44bf088528fd79a7e5d09666006ee07760c39c",
        "worker_id": "a"
      },
      "kind": "READY",
      "previous": null,
    ...
    },
    {
      "detail": {
        "phase_id": "ad2278ae871a4f6f8e32c86fe835ca88",
        "reply_digest": "sha256:48302b162c8561c1c150a6e61cb7299e23137011b059edd02053c4592d19f4cb",
        "worker_id": "b"
      },
      "kind": "READY",
      "previous": "sha256:3199400944ceda2aae967e6a4ecf3661431b90d182945580c51c90363cee5a3a",
      "run_id": "c15397adfca4466d91f2663a258c4135"
    },
    {
      "detail": {
        "phase_id": "ad2278ae871a4f6f8e32c86fe835ca88",
        "reply_digest": "sha256:105b71393a87c37885cf2b1312a097fa98ff8d8bdfc09de7c01df93b3a812eef",
        "worker_id": "c"
      },
      "kind": "READY",
      "previous": "sha256:88cc2c8bc3a4afd663a55e79d9a19cbae30983d040138b45329297b7d0d38276",
      "run_id": "c15397adfca4466d91f2663a258c4135"
    },
    {
      "detail": {
      ...
      "kind": "PHASE_CHECK",
      "previous": "sha256:3aa85645d6d4b840b1c803ab590184b391bde531c6b540da2bab90caf91c8925",
      "run_id": "c15397adfca4466d91f2663a258c4135"
    },
    {
      "detail": {
      ...
      "kind": "COMPARISON_STARTED",
      "previous": "sha256:4430b32decb581efbc52fa43d3674d492daaa51420a0ac8339a6ef23243c6fd2",
      "run_id": "c15397adfca4466d91f2663a258c4135"
    },
    {
      "detail": {
      ...
      "kind": "COMPARISON_VALIDATED",
      "previous": "sha256:09a11a36a92f4e7be9b71bf236e3e9f0ddf611d26cb6fa19252893a0c18be827",
      "run_id": "c15397adfca4466d91f2663a258c4135"
    },
    {
      "detail": {
      ...
      "kind": "SEMANTIC_REVIEW_STARTED",
      "previous": "sha256:08637377ab6d754b47d0622d78d580efabedf73fc2f469ba3f5aed4ebae03335",
      "run_id": "c15397adfca4466d91f2663a258c4135"
    },
    {
      "detail": {
      ...
      "previous": "sha256:1df84af491cb697c123328bfe848a6c769cf88bc47c1ee7729166b9b21a162f4",
      "run_id": "c15397adfca4466d91f2663a258c4135"
    },
    {
      "detail": {
        "result_digest": "sha256:b85d0d346d54fcfe3cf24278e101b77840c9ca59a8ce8ef93f6f6cc16021136d"
      },
      "kind": "LOCAL_CHECKS_PASSED",
      "previous": "sha256:abccef61e212cb2ff0d4fe90f592535f7b3b97cc51ebe279adbf3b28cadb8eb6",
      "run_id": "c15397adfca4466d91f2663a258c4135"
    }
  ],
  "production_approval": false
...
[
  {
    ...
    "exit_code": 0,
    "status": "LOCAL_CHECKS_PASSED",
    "expected_status": "LOCAL_CHECKS_PASSED",
    "reason": null,
    "events": [
      "READY",
      "READY",
...
```json
{
  ...
  "claims": [
    ...
    },
    {
      ...
      "verdict": "SUPPORTED",
      "url": "https://developers.openai.com/api/docs/models/gpt-6-astra",
      "location": "Model introduction and supported effort list; checked in previous audit this session"
    }
  ],
  "limits": [
    "Documentation was inspected; it does not prove a live provider call occurred.",
    "HTTP response fixtures tested the adapter contract only.",
    "Page publication/as_of times unknown; no future freshness guarantee."
  ],
  "model_eval": "NOT_RUN",
...
```json
{
  ...
  "skipped": 0,
  "successful": true,
  "elapsed_seconds": 10.202292221998505,
  "python_version": "3.12.13",
  "finished_at": "2026-09-06T12:36:27.310091+00:00",
  "mode": "LOCAL_TEST",
  "model_eval": "NOT_RUN",
  "live_openai_call": false,
  "production_approval": false,
  "scope": "Migrated original structural tests, new mandatory host gates and HTTP response fixtures"
}
...
test_lexically_matching_wrong_meaning_rejects_reviewer_verdict (test_host_integration.HostIntegrationTests.test_lexically_matching_wrong_meaning_rejects_reviewer_verdict) ... ok
